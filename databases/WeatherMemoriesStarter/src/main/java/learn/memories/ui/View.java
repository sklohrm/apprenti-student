...
public class View {
    ...
    /**
     * Prompts the user for login credentials and returns a User object
     * containing the entered username (String) and password (byte[]).
     * @param message The header message/prompt to display.
     * @return A User object populated with credentials for a login attempt.
     */
    public User userLogin(String message) {
        displayHeader(message);

        String username = io.readString("Username: ");

        // NOTE: In a secure application, the password input should be masked.
        String passwordString = io.readString("Password: ");

        // Convert the plaintext password string to a byte array using UTF-8 encoding
        byte[] passwordBytes = passwordString.getBytes(StandardCharsets.UTF_8);

        // Populate and return the User object
        User loginAttempt = new User();
        loginAttempt.setUserName(username);
        // Assuming the User model has a setPassword(byte[] password) method
        loginAttempt.setPassword(passwordBytes);
